...
shipper: build/shipper Dockerfile.shipper
	docker build -f Dockerfile.shipper -t $(SHIPPER_IMAGE) --build-arg HTTP_PROXY=$(HTTP_PROXY) --build-arg HTTPS_PROXY=$(HTTPS_PROXY) build/
	docker push $(SHIPPER_IMAGE)

shipper-state-metrics: build/shipper-state-metrics Dockerfile.shipper-state-metrics
	docker build -f Dockerfile.shipper-state-metrics -t $(METRICS_IMAGE) --build-arg HTTP_PROXY=$(HTTP_PROXY) --build-arg HTTPS_PROXY=$(HTTPS_PROXY) build/
...
lint:
	golangci-lint run --config .golangci.yml ./pkg/... ./cmd/... ./test/...

test:
	go test -v ./pkg/... ./cmd/... ./test/...
